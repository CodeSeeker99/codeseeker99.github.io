...
**Finally**, while our exposure model is performing very well, our blur model gives exaggerated outputs. I believe this can be fixed using a different threshold for classification. For example, we could change the blurriness threshold classify as blurry only above 75%. Now, using this function, we could rename/shift the files according to our needs. This would save some serious time while sorting through clips, and perhaps have a real impact on large scale video production. 
